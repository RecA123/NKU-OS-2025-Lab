...
- **页层（`page_area`）** 负责物理页粒度的管理，使用“best_fit”策略维护空闲块（`struct Page` 的 `property` 字段记录连续页数；链表按物理地址升序排列）。
...
SLUB 原理
  ...
  - 这种 slab 化策略让同尺寸运行时复用同一页，避免反复在页层拆分，同时 SLUB_SLAB_MAGIC 等校验保证误用时立即暴露问题。check函数覆盖
